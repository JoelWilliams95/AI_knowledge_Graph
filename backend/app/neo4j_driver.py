...
def upsert_paper(paper_id: str, filename: str, title: str, text: str, metadata: Dict[str, Any] = None):
    ...
    with driver.session() as session:
        props = {
            "paper_id": paper_id,
            "id": paper_id,  # Add 'id' for consistency with entities
            "name": title,  # Add 'name' as title for consistency
            "filename": filename,
            "title": title,
            "text": text,
...
def get_graph(limit: int = 100):
    ...
    with driver.session() as session:
        q = (
            "MATCH (n)-[r]-(m) "
            "WHERE NOT 'Paper' IN labels(n) AND NOT 'Paper' IN labels(m) "  # Skip Paper nodes
...
def get_subgraph(center_id: str, depth: int = 1):
    ...
    with driver.session() as session:
        q = (
            ...
            "WHERE NOT 'Paper' IN labels(c) AND NOT 'Paper' IN labels(n) "
        ...
        for record in res:
            ...
            if rel is not None:
                ...
                if start_id and end_id and start_id in nodes and end_id in nodes:
                    edge_id = str(rel.id)
...
def search_entities(query: str, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    with driver.session() as session:
        cypher = """
        MATCH (e)
        WHERE (e:Entity OR e:PERSON OR e:ORG OR e:GPE OR e:WORK_OF_ART OR e:CONCEPT)
